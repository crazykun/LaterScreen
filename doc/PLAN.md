...
现状：工具栏用中文文字标签（`toolbar.rs` TOOLS 表），9 个工具 + 撤销/重做/保存/复制/二维码/OCR
共 15 个文字按钮，横向占用超过 620pt，小屏或窄选区下挤压严重。

- [ ] 工具栏按钮改为图标 + hover tooltip（tooltip 复用现有 `on_hover_text` 文案，
      补上快捷键提示）
- [ ] 图标方案：**egui 内置字体的 Unicode 符号优先**（零体积零依赖，符合体积硬约束）；
      符号辨识度不足的工具（马赛克/标号/OCR）改为 `Painter` 手绘 12×12 矢量图标
      （纯几何绘制，同样零资源文件）。不引入 SVG 库、不打包 PNG 图集
- [ ] 按钮尺寸统一 28×28，分组间距收敛，目标整条工具栏 ≤ 400pt
- [ ] 颜色选择器改为单个当前色按钮 + 点击展开调色板 popup，省掉常驻 8 个色块
- [ ] 无障碍：每个图标按钮设置 `Response::on_hover_text` 与语义 label，
      保持键盘可达（Tab 序）
