...
1. **双渲染路径**：交互期用 `egui::Painter` 实时绘制（GPU/glow），导出时用
   `tiny-skia` 在 core 内软渲染合成到截图上。两者共享同一份图元数据，
   保证所见即所得。代价是每种图元写两份绘制代码——用 `Element` 上的统一
   几何描述把重复压到最低。
...
| 风险 | 影响 | 对策 |
|---|---|---|
| Wayland 禁止直接抓屏 | Linux 部分桌面不可用 | xcap 走 portal；X11 优先支持；边缘 compositor 明确声明不支持 |
| "用完即走"与全局快捷键矛盾 | 无法自监听热键 | 快捷键绑定交给系统/桌面环境，绑定到 `lscreen` 命令；编辑期快捷键由 egui 处理 |
| 纯 Rust 无 H.264 编码器 | MP4 依赖问题 | 系统编码器；GIF 先行 |
