...
不带参数运行即静默驻留后台：托盘菜单（截图 / 取色 / 贴图 / 录屏 / 滚动截图 / 历史 / 配置 / 退出）+ 全局热键随时唤起，默认 **F1 截图**。**「历史」打开一个缩略图面板浮窗**（Snipaste 同款思路：原生菜单画不了缩略图，自绘窗口展示最近截图/贴图/录屏，点击复制；录屏点击用默认播放器播放视频；右键可贴图/打开目录/删除）。每个动作是独立子进程，用完即退，主进程始终轻量。
...
| `config` | — | 打开配置面板 |
| `history` | — | 打开截图历史面板（最近截图/贴图/录屏，缩略图网格，点击复制、录屏播放） |
