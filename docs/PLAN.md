...
- [x] 滚动截图：`lscreen scroll`（托盘菜单「滚动截图」同入口）。
      ...
      另一限制：选区底部压着高度 ≥ 匹配块的固定内容（输入条/工具栏/
      固定页脚）时尾块每步匹配在原位 → 恒 NoChange（上层以
      「画面在滚动但拼接无进展」归因提示，与滚轮未生效的「画面无变化」
      区分，LSCREEN_SCROLL_DEBUG=1 可看逐步跟踪）。
      真机 E2E 受宿主终端滚轮行为干扰（xterm 对持续输出会自动跳回底部
      scrollTtyOutput、alt-screen 不转发滚轮），理想宿主是浏览器/编辑器
      mac 真机点验（2026-09-19，随 v0.8.0 三平台化后补验）修 5 处：
      ① `scroll_wheel` 行单位不可用——浏览器对「行」的解释在 Retina 下
      可达 40+ 逻辑px，一步位移远超尾块匹配窗，内容整窗滚飞 → 全程
      NoChange；改像素单位（每 notch 15 逻辑px）。② 「自然滚动」开启时
      会话层注入的滚轮增量被系统再翻转——按 com.apple.swipescrolldirection
      偏好预翻转符号。③ mac/Win 浏览器平滑滚动是惯性动画，固定 pause
      截到中间态 → 尾块错位重影；改 `wait_stable` 轮询等画面收尾
      （预算 pause×4 钳 1.2s，页面有持续动画时不无限等）。④ 加 armed
      阶段（「开始滚动」Enter），用户先把目标窗口摆好、定位到内容顶部。
      ⑤ 状态窗必须显式摆位：winit 在 mac 对未指定位置的窗口一律
      `center()`，置顶状态窗恰盖住屏幕中心——交互框选的区域中心常在
      同一点，warp 后合成滚轮按「指针下最顶层窗口」全被状态窗吃掉
      （egui 不滚动不透传）→ 未检测到滚动退化普通截图；capture 层
      `status_window_origin` 在选区所在屏按 右→左→下→上 挑不重叠位置。
      ⑥ 浏览器对合成滚轮有加速放大（真机实测 1 格名义 15 逻辑px 实际位移
      ~168 物理px），矮选区单步位移超过「首帧高−匹配块」即尾块整块滚出
      画面 → 首步 Mismatch；滚动循环改闭环校准：首步恒 1 格实测位移，
      之后按预算一半自适应格数（--clicks 变为上限），Appended 行数即实测
      位移。⑦ 亚像素滚动偏移 + 帧缝：平滑滚动常停在亚像素位置，文字边缘
      AA 逐帧微变，精确行匹配时灵时不灵（真机探针：同区域同格数因尾块
      落点不同分别得 Appended(168)/Appended(16)/Mismatch）——签名加
      [1,2,1] 垂直平滑吸收 ±1px 漂移；帧末行的平滑在帧内用边界复制而
      下一帧同内容行带真实下邻，两处平滑必然不同 → 锚行改取**倒数第二
      行**（缝行留在块内参与整块 SAD，误差 1/k 稀释）。真机两帧探针
      （`scroll_two_frame_probe`，LSCREEN_TEST_E2E=1）可复现诊断：页底
      再滚位移为 0 且无伪影，NoChange 语义正确。⑧ armed 期间用户定位
      内容的手动滚动带惯性（~2s）——滚完立刻点「开始滚动」时首步位移 =
      惯性剩余 + 合成位移，必超预算 Mismatch；开始后先 wait_stable
      （预算 2.5s）等画面静止再采首帧，状态窗提示「等滚动停稳再开始」。
      ⑨ capture_region 的逻辑坐标取整会把采帧矩形外扩 ≤1 逻辑px，贴着
      选区的录制红条被采进每帧 → 长图每隔一步一条红线；mac 边条与选区
      间加 G=6 物理px 空隙（大于取整误差）。
...
- [x] **区域穿透 + 图标重绘（2026-09-23，用户反馈）**：穿透升级为
      「图像区穿透、工具条条带保留交互」——开启后图像区点击落到下层
      窗口，条带（穿透按钮/缩放/关闭等）仍可点，**再点穿透按钮即恢复**
      （用户主诉：原来开启即整窗穿透，按钮再也点不到，只能 Esc/托盘）。
      实现：`set_click_through` 增加 `keep: Option<(x,y,w,h)>`（物理像素
      保留区），X11 用 `shape_rectangles` 设单矩形输入区（空列表=整窗
      穿透、ShapeMask(None)=恢复，语义见 M12 上文）；穿透期窗口尺寸
      变化（条带按钮仍可点 → 缩放/旋转会改 InnerSize）后条带位置随之
      变化，`through_shape` 指纹逐帧比对重下发（请求的 InnerSize 下一
      帧才反映到 viewport，靠 300ms 心跳帧追平）。条带矩形换算
      `bar_strip_physical`（逻辑 inner_rect × pixels_per_point，窗口比
      条带矮时钳顶）。Win/mac 平台原语只有整窗语义（`WS_EX_TRANSPARENT`
      / `setIgnoresMouseEvents`，区域方案需子窗口/消息子类化，成本不成
      比例不做），`input_region_supported()` 区分，行为与文案保持整窗
      穿透不变。穿透图标重绘：窗口轮廓+横穿箭头 → **圆盾 + 斜射穿盾的
      箭**（用户反馈原图标太丑；箭为主角——大箭头+尾羽开叉，盾心坐
      箭杆上偏尾侧，~40° 避开禁止符号 ⊘ 的 45°），icon_preview 离屏
      渲染核对通过。
- [x] **贴图真置顶修复（同轮用户反馈「点穿透就后台了」）**：根因不是
      穿透——`with_always_on_top` 建窗选项在部分 WM（Deepin kwin 实测）
      **不落地**：贴图从未持有 `_NET_WM_STATE_ABOVE`，只靠「最新激活的
      普通窗口」假性置顶。穿透后点击落到下层窗口（激活+raise）即被盖
      住；点条带取消穿透时贴图被重新激活回最上——与用户「再取消再开
      就不影响」的观察完全吻合。v0.9 起就存在，仅穿透场景暴露。修复：
      `ping_window_level` 建窗后分 3 次（0/0.3/0.9s）重发
      `ViewportCommand::WindowLevel(AlwaysOnTop)`——egui-winit/winit X11
      对该命令均无去重、每次真发 EWMH 消息（幂等），且已 map 窗口上
      实测有效（置顶按钮手动切换路径本就工作）；补发期间主动
      request_repaint（贴图无输入事件时 eframe 不重绘，定时发送不触发）。
      真机端到端验证：新建贴图 xprop 即见 ABOVE+STAYS_ON_TOP（零点击）；
      windowactivate 激活下层窗口后堆叠不变（贴图仍最高）；穿透输入区
      指针探测：图像区命中下层窗口、条带命中贴图自身。
